...
@author: FrankNGUEN
http://ainoodle.tech/2020/02/13/phat-hien-lan-duong-trong-bai-toan-xe-tu-lai/
"""
#------------------------------------------------------------------------------
# import thu vien
#------------------------------------------------------------------------------
...
def findLaneLines(crop, edged):
    lines       = cv2.HoughLines(edged, 1, np.pi / 180, 25)      # Tim tat ca cac lines trong hinh
    rho_left    = []                                             # Dinh nghia cac bien luu
    theta_left  = []
    rho_right   = []
    theta_right = []
    if lines is not None:                                        # Neu nhu tim duoc lines
        # Lap qua cac line
...
while video.isOpened():
    ...
    crop    = frame[350:550, 300:950]                   # Crop and display main ROI view of video
    ...
    thresh  = 80                                        # Apply threshold 
    frame   = cv2.threshold(frame, thresh, 255, cv2.THRESH_BINARY)[1]
    blurred = cv2.GaussianBlur(frame, (13, 13), 0)
    edged   = cv2.Canny(blurred, 30, 150)               # Find edge
    crop    = findLaneLines(crop, edged)                # perform find lane lines
    cv2.imshow("Output", crop)
    if cv2.waitKey(1) & 0xFF == ord('q'):
        break
video.release()                                         # clear everything once finished 
